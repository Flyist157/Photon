...
import type { CapturedFrame, RoomModel, StyledPhoto } from './types'
...
function App() {
  ...
  const processScan = async (): Promise<void> => {
    ...
    try {
      ...
      const listingPhotos = generateListingPhotos(model)
      setRoomModel(model)
    ...
    <main className="app-shell">
      <header className="card">
        ...
        <p>
          ...
          Photon captures high-res room views, reconstructs a depth-sensitive model, then synthesizes three
      ...
      {stage === 'processing' && (
        <section className="card center">
          <h2>2) Reconstructing room model</h2>
          <p>Compiling color + depth cues, then rendering three listing angles with post-production polish.</p>
        </section>
      )}
